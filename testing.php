...
// Get ETH data
$select="SELECT * FROM eth ORDER BY lastUpdate DESC Limit 1";
$result = $sql->query($select);
...
if((isset($_POST['newGpuName']) || isset($_POST['delete'])) && $_POST['randcheck']==$_SESSION['rand']) {
    ...
    if(isset($_POST['newGpuName'])) {
        ...
        $gpuList = array_map('str_getcsv', file('gpus.csv'));
        array_walk($gpuList, function(&$a) use ($gpuList) {
            $a = array_combine($gpuList[0], $a);
        });
        ...
        $insert = "INSERT INTO `gpus` (`id`, `name`, `price`, `mhs`, `watt`) VALUES
        ('".$_POST['newGpuId']."','".$_POST['newGpuName']."',".$_POST['newGpuPrice'].",".$_POST['newGpuHashrate'].",".$_POST['newGpuWatt'].")
        ";
...
#    echo "The number is: $x <br>";
    $gpuList[$counter] += [
        "ethPerDay" => ($gpuList[$counter]['mhs']/1000/($row['networkHashRate']/1000000000))*(3600/$row['blockTime']*$row['rewardBlock']*24)*(1-($poolfee/100)),
        "powerCostPerDay" => ceil($gpuList[$counter]['watt']*24/1000*$powerCost*100)/100,
    ];
    $gpuList[$counter] += [
        "revenuePerDay" => round($gpuList[$counter]['ethPerDay']*$row['ethUsd'],2),
        "revenueCHFPerDay" => round($gpuList[$counter]['ethPerDay']*$row['ethUsd']*$exchange_rate_usd_chf,2),
        "profitPerDay" => round($gpuList[$counter]['ethPerDay']*$row['ethUsd']-$gpuList[$counter]['powerCostPerDay'],2),
        "profitCHFPerDay" => round($gpuList[$counter]['ethPerDay']*$row['ethUsd']*$exchange_rate_usd_chf-$gpuList[$counter]['powerCostPerDay'],2),
    ];
}

// Market conditions:

// Get AVG(ethUSD) from yesterday
$select="select AVG(ethUsd) as ethUsd, WEEK(lastUpdate) as lastweek from eth GROUP BY lastweek ORDER BY lastweek desc limit 1, 1";
$result = $sql->query($select);
$ethPriceLastWeek = $result->fetch_assoc();

// Green Market
if($row['ethUsd'] >= $ethPriceLastWeek['ethUsd']) {
    if($row['ethUsd'] >= $ethPriceLastWeek['ethUsd']*1.1) {
...
// Red Market
    ...
    if($row['ethUsd'] <= $ethPriceLastWeek['ethUsd']*0.9) {
...
<form action="/" method="post">
    ...
    // Warning when eth block reward is unusally high
    if($row['rewardBlock']>3) {
    ?>
        <div class="alert alert-warning" role="alert">
        <b>Careful!</b> Unusually high Etherum block rewards at the moment: <?php echo $row['rewardBlock']; ?><br>
        Etherum block rewards are usually around 2.2<br>
        <br>
    ...
    <div class="container content">
        <div class="col-sm-9 infobox">
            <div>
                ...
                <div class="col-sm-2">ETH price</div><div class="col-sm-3 infofixtext"><div class="input_units col-sm-10"><?php echo $row['ethUsd']; ?><label class="units">USD</label></div><img class="col-sm-2" width="20px" src="<?php echo $marketcolor; ?>" title="Average ETH price last week: $<?php echo round($ethPriceLastWeek['ethUsd'],2); ?>"/></div><div class="col-sm-7 infofixtext">(Last update: <?php echo $row['lastUpdate']; ?>)</div>
...
?>
                ...
                <p class="hover a19 sidetitle">Efficiency ETH / Watt</p>
        ...
        <div class="col-sm-9 scrollcontent">
            <table class="table cryptosheet">
                ...
                <tr class="hover b15">
                    ...
                    for ($counter = 0; $counter < sizeof($gpuList); $counter++) {
                        echo "<td style='background-color:".colorGradient((${$yearRev.$gpuList[$counter]['id']}-$revYearMin)/$revYearDiff)."'>".${$yearRev.$gpuList[$counter]['id']}."%</td>";
                    }
                    ?>
                </tr>
                <tr class="hover b16">
    ...
    <figure><pre>
ETH Network Hashrate:   <?php echo $row['networkHashRate']/1000000000000; ?> TH/s 
Mining Block Time:      <?php echo $row['blockTime']; ?>s 
ETH Price:              $<?php echo $row['ethUsd']; ?> 
Mining Difficulty:      <?php echo $row['difficulty']; ?> 
Block Reward:           <?php echo $row['rewardBlock']; ?> ETH</pre></figure>
    </div>
    <div class="content">
        <p style="margin-bottom:10px;"><strong>To Do:</strong></p>
    <figure><pre>
- Fix missing axis numbers in price trend line
- Optimize display of GPU price trend line
- Add option to delete name from GPU name dictionary</pre></figure>
    </div>
</form>
</div>
