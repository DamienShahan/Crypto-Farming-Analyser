...
#    echo "The number is: $x <br>";
    $gpuList[$counter] += [
        "coinPerDay" => ($gpuList[$counter]['mhs']/1000/($row['networkHashRate']/1000000000))*(3600/$row['blockTime']*$row['rewardBlock']*24)*(1-($poolfee/100)),
        "powerCostPerDay" => ceil($gpuList[$counter]['watt']*24/1000*$powerCost*100)/100,
    ];
    $gpuList[$counter] += [
        "revenuePerDay" => round($gpuList[$counter]['coinPerDay']*$row['etcUsd'],2),
        "revenueCHFPerDay" => round($gpuList[$counter]['coinPerDay']*$row['etcUsd']*$exchange_rate_usd_chf,2),
        "profitPerDay" => round($gpuList[$counter]['coinPerDay']*$row['etcUsd']-$gpuList[$counter]['powerCostPerDay'],2),
        "profitCHFPerDay" => round($gpuList[$counter]['coinPerDay']*$row['etcUsd']*$exchange_rate_usd_chf-$gpuList[$counter]['powerCostPerDay'],2),
    ];
}

// Market conditions:

// Get AVG(etcUSD) from yesterday
$select="select AVG(etcUsd) as etcUsd, WEEK(lastUpdate) as lastweek from etc GROUP BY lastweek ORDER BY lastweek desc limit 1, 1";
$result = $sql->query($select);
$etcPriceLastWeek = $result->fetch_assoc();

// Green Market
if($row['etcUsd'] >= $etcPriceLastWeek['etcUsd']) {
    if($row['etcUsd'] >= $etcPriceLastWeek['etcUsd']*1.05) {
...
// Red Market
    ...
    if($row['etcUsd'] <= $etcPriceLastWeek['etcUsd']*0.95) {
...
<form action="/" method="post">
    ...
    // Warning when etc block reward is unusally high
    if($row['rewardBlock']>3) {
    ?>
        <div class="alert alert-warning" role="alert">
        <b>Careful!</b> Unusually high ETC block rewards at the moment: <?php echo $row['rewardBlock']; ?><br>
        ETC block rewards are usually around 2.2<br>
        <br>
    ...
    <div class="container content">
        <div class="col-sm-9 infobox">
            <div>
                ...
                <div class="col-sm-2">ETC price</div><div class="col-sm-3 infofixtext"><div class="input_units col-sm-10"><?php echo $row['etcUsd']; ?><label class="units">USD</label></div><img class="col-sm-2" width="20px" src="<?php echo $marketcolor; ?>" title="Average etc price last week: $<?php echo round($etcPriceLastWeek['etcUsd'],2); ?>"/></div><div class="col-sm-7 infofixtext">(Last update: <?php echo $row['lastUpdate']; ?>)</div>
...
?>
                ...
                <p class="hover a19 sidetitle">Efficiency ETC / Watt</p>
    ...
    <figure><pre>
ETC Network Hashrate:   <?php echo $row['networkHashRate']/1000000000000; ?> TH/s 
Mining Block Time:      <?php echo $row['blockTime']; ?>s 
ETC Price:              $<?php echo $row['etcUsd']; ?> 
Mining Difficulty:      <?php echo $row['difficulty']; ?> 
Block Reward:           <?php echo $row['rewardBlock']; ?> etc</pre></figure>
    </div>
    <div class="content">
...
- Check if it is worth more to payout ETC directly to CHF, instead of ETC->USD->CHF</pre></figure>
    </div>
</form>
</div>
